...
$(function(){
	var $ckeditor = $("textarea[name='editor1']").ckeditor();
	var $articleContent = $('input[name="article.content"]');
	//submit Button添加事件
	$('#save').on('click', function(){
		$articleContent.val($ckeditor.val());
		$('#form1').submit();
	});
...
$("#articles").addClass("current");$("#article_add").addClass("current");</script>	
			<div class="content-box"><!-- Start Content Box -->
				...
				<s:form id="form1" name="form1" action="/article/saveArticle.html"
					...
					<div class="tab-content  default-tab" id="tab1">
						<fieldset>
							<p>
								<label>标题</label>
								<s:textfield name="article.title"
									cssClass="text-input medium-input" id="title" value="" />
							</p>
							<p>
								<label>分类</label> <span id='typeidct'> 
                                 	  <s:select name="article.articleType" cssClass="select" list="%{@com.sut.util.meta.ArticleTypeEnum@getAll()}" listKey="stringValue" listValue="displayText">
                                 	  </s:select> 
								</span>
							</p>
							<p>
								<label>缩略图</label> 
								<s:textfield name="article.thumbnail" id="thumbnail" style="width: 300px" value="" />
								<input type="button" name="thumbnail.button" class="button" value="上传">
							</p>
							<p>
								<label>简介</label>
								<s:textarea name="article.summary" rows="5" id="description"
									style="width: 80%; height: 50px" />
							</p>
							<p>
								<label>内容</label>
								<textarea name="editor1" id="editor1" rows=10 cols="80">
										This is my textarea to be replaced by ckeditor.
									</textarea>
								<s:hidden type="textarea" name="article.content"></s:hidden>
							</p>
							<p>
								<br />
